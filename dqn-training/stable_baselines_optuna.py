...
def train_with_eval_every_n_steps(problem, instance, lr, batch_size, eps_final, gamma, buffer_size, 
          ...
          load_replay_buffer=False, n_steps=10000):
    ...
    model = DoubleDQN('MultiInputPolicy', env, **kwargs)
    ...
    callback = CustomEvalAndPlotCallback(
        ...
        plot_fn=plot,
        plot_path=f"plots_double_dqn/{problem}_{instance}/lr_{lr}_eps_final_{eps_final}.png",
        n_steps=n_steps
    )
    ...
    with open(f"tuned_eval_stats.txt", "a+") as f:
...
def objective(trial, problem, instance, steps=1000000, load_replay_buffer=False):
    ...
    eval_stats = train_with_eval_every_n_steps(
        ...
        gradient_steps, max_grad_norm, steps, load_replay_buffer)

    return eval_stats['mean']


def optimize(problem, instance, steps=1000000):
